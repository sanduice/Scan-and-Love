...
import { Star, Upload, Palette, Clock, Truck, Shield, ChevronDown, ChevronUp, Check, Info, Loader2, Ruler, X, ShoppingCart, FileText } from 'lucide-react';
...
const STANDARD_SIZES = {
  ...
  'table-throw': [{
    ...
    width: 126,
    height: 84,
    key: '6ft Table Throw (3-sided)'
  }, {
    label: '6ft (4-sided)',
    width: 126,
    height: 84,
    key: '6ft Table Throw (4-sided)'
  }, {
    label: '8ft (4-sided)',
    width: 150,
    height: 90,
    key: '8ft Table Throw (4-sided)'
  }],
  'pop-up-display': [{
    label: '8ft Straight',
    width: 90,
...
export default function ProductConfigurator({
  ...
  const calculatedPrice = useMemo(() => {
    ...
    if (!product.product_options || product.product_options.length === 0) {
      ...
      const modifiers = getModifier(product.finish_options, selectedOptions.finish) + getModifier(product.grommet_options, selectedOptions.grommets) + getModifier(product.pole_pocket_options, selectedOptions.polePockets);
      unitPrice += modifiers;
  ...
  const standardSizes = useMemo(() => {
    ...
    if (product.preset_sizes && Array.isArray(product.preset_sizes) && product.preset_sizes.length > 0) {
      return product.preset_sizes.filter(size => size.is_active !== false).map(size => ({
  ...
  const handleAddToCartWithUploads = async () => {
    ...
    try {
      ...
      await base44.entities.SavedDesign.create({
        ...
        is_in_cart: true,
        artwork_url: uploadedFiles[0].url,
        // Primary artwork
  ...
  const renderOptionGroup = (label, options, selectedKey, optionKey) => {
    ...
    return <div className="py-4 border-b border-gray-100">
        <div className="flex items-center justify-between mb-3">
          <span className="text-sm font-medium text-gray-700">{label}:</span>
          <span className="text-sm text-blue-600 font-medium">{selectedOptions[optionKey]}</span>
        </div>
        <div className="grid grid-cols-2 gap-2">
          {options.map(option => <button key={option.name} onClick={() => updateOption(optionKey, option.name)} className={`p-3 text-left rounded-lg border-2 transition-all ${selectedOptions[optionKey] === option.name ? 'border-blue-500 bg-blue-50' : 'border-gray-200 hover:border-gray-300'}`}>
              <div className="font-medium text-sm">{option.name}</div>
              {option.description && <div className="text-xs text-gray-500 mt-1">{option.description}</div>}
              {(option.price_modifier > 0 || option.price > 0) && <div className="text-xs text-green-600 mt-1">+${option.price_modifier || option.price}</div>}
            </button>)}
        </div>
      </div>;
  };
  ...
  const renderDynamicProductOptions = () => {
    ...
    return options.filter(group => group.is_visible !== false).map(group => {
      const visibleChoices = (group.choices || []).filter(c => c.is_visible !== false);
      ...
      return <div key={group.id} className="py-4 border-b border-gray-100">
            <div className="flex items-center justify-between mb-3">
            ...
            <div className="grid gap-2 grid-cols-2">
              {visibleChoices.map(choice => <button key={choice.id} onClick={() => updateDynamicOption(group.id, choice)} className={`p-3 text-left rounded-lg border-2 transition-all flex items-start gap-3 ${selectedChoice?.id === choice.id ? 'border-blue-500 bg-blue-50' : 'border-gray-200 hover:border-gray-300'}`}>
                  {choice.image_url && <div className="w-14 h-14 overflow-hidden rounded-md flex-shrink-0">
                      <img src={choice.image_url} alt={choice.title} className="w-full h-full object-cover" />
                    </div>}
                  <div className="flex-1 min-w-0">
                    <div className="font-medium text-sm">{choice.title}</div>
                    {choice.hint && <div className="text-xs text-muted-foreground mt-1">{choice.hint}</div>}
                    {parseFloat(choice.price) > 0 && <div className="text-xs text-green-600 mt-1">+${parseFloat(choice.price).toFixed(2)}</div>}
                  </div>
                </button>)}
            </div>
          </div>;
  ...
  return <div className="space-y-6">
      ...
      <div>
        ...
        <div className="flex items-center gap-4 mt-3">
          <div className="flex items-center gap-1">
            {[...Array(5)].map((_, i) => <Star key={i} className={`w-5 h-5 ${i < Math.floor(product.average_rating || 4.5) ? 'text-yellow-400 fill-yellow-400' : 'text-gray-300'}`} />)}
          </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
        ...
        {hasAnyImages ? (/* Image Card Mode - when sizes have images */
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3 mb-4">
            {standardSizes.map(size => {
          const isSelected = !isCustomSize && width === size.width && height === size.height && (size.key ? sizeKey === size.key : true);
          return <button key={size.label} onClick={() => {
            setWidth(size.width);
            setHeight(size.height);
            setSizeKey(size.key || null);
            setSelectedPresetPrice(size.price !== undefined ? size.price : null);
            setIsCustomSize(false);
          }} className={`p-3 rounded-xl border-2 transition-all flex flex-col items-center text-center ${isSelected ? 'border-blue-500 bg-blue-50' : 'border-gray-200 hover:border-gray-300 bg-white'}`}>
                  {size.image_url ? <img src={size.image_url} alt={size.label} className="w-16 h-16 object-cover rounded-lg mb-2" /> : <div className="w-16 h-16 bg-gray-100 rounded-lg mb-2 flex items-center justify-center">
                      <Ruler className="w-6 h-6 text-gray-400" />
                    </div>}
                  <span className="font-medium text-sm text-gray-900">{size.label}</span>
                  {size.price !== undefined && <span className="text-xs text-muted-foreground mt-0.5">${parseFloat(size.price).toFixed(2)}</span>}
                </button>;
        })}
            {allowCustomSize && <button onClick={() => {
          setIsCustomSize(true);
          setSizeKey(null);
          setSelectedPresetPrice(null);
        }} className={`p-3 rounded-xl border-2 transition-all flex flex-col items-center justify-center text-center ${isCustomSize ? 'border-blue-500 bg-blue-50' : 'border-gray-200 hover:border-gray-300 bg-white'}`}>
          ...
          </div>) : (/* Text Mode - compact buttons when no images */
      <div className="flex flex-wrap gap-2 mb-4">
            {standardSizes.map(size => <button key={size.label} onClick={() => {
          setWidth(size.width);
          setHeight(size.height);
          setSizeKey(size.key || null);
          setSelectedPresetPrice(size.price !== undefined ? size.price : null);
          setIsCustomSize(false);
        }} className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${!isCustomSize && width === size.width && height === size.height && (size.key ? sizeKey === size.key : true) ? 'bg-blue-500 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}>
                {size.label}
              </button>)}
            {allowCustomSize && <button onClick={() => {
          setIsCustomSize(true);
          setSizeKey(null);
          setSelectedPresetPrice(null);
        }} className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${isCustomSize ? 'bg-blue-500 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}>
                Custom
              </button>}
          </div>)}
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          }} disabled={!allowCustomSize && !isCustomSize} className="mt-1 h-12 text-lg disabled:opacity-50 disabled:bg-gray-100" />
          </div>
          <div>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
        <Collapsible open={openSections.options} onOpenChange={() => toggleSection('options')}>
          ...
          <CollapsibleContent className="px-6 pb-6">
            {product.product_options && product.product_options.length > 0 ? renderDynamicProductOptions() : <>
              ...
              name: 'Welded Hem',
              description: 'Heat welded for durability'
            }, {
              name: 'Sewn Hem',
              description: 'Sewn edges for extra strength',
              price_modifier: 5
            }, {
              name: 'None (Flush Cut)',
              description: 'No hem, exact size cut'
            }], selectedOptions.finish, 'finish')}
                
                {!product.hidden_options?.includes('grommets') && renderOptionGroup('Grommets', product.grommet_options || [{
              name: 'Every 2-3 ft',
              description: 'Corners + evenly spaced'
            }, {
              name: 'Every 12-18 in',
              description: 'More hanging points'
            }, {
              name: '4 Corners',
              description: 'Just corner grommets'
            }, {
              name: 'None',
              ...
              name: 'None',
              description: 'No pole pockets'
            }, {
              name: '3" Top & Bottom',
              description: 'Fits 1.5" poles',
              price_modifier: 8
            }, {
              name: '3" Top Only',
              description: 'Top pocket only',
              price_modifier: 5
              ...
              name: 'Bungees (8)',
              description: '12" rubber bungee cords',
              price: 9.99
            }, {
              name: 'Zip Ties (10)',
              description: 'Black zip ties',
              price: 4.99
            }, {
              name: 'Rope (4)',
              description: '10ft nylon rope pieces',
              price: 12.99
            }, {
              name: 'Hanging Clips (6)',
              description: 'Steel carabiners',
              price: 14.99
            }, {
              name: 'Suction Cups (8)',
              description: '2" cups with hooks',
              price: 11.99
      ...
      {/* Design Method Card */}
      {product.has_design_tool !== false && <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
          <h3 className="font-semibold text-gray-900 mb-4">Select Your Design Method</h3>
          <div className="grid grid-cols-2 gap-3">
            <button onClick={() => setDesignMethod('design-online')} className={`p-4 rounded-xl border-2 transition-all text-center ${designMethod === 'design-online' ? 'border-blue-500 bg-blue-50' : 'border-gray-200 hover:border-gray-300 bg-white'}`}>
              <div className="flex flex-col items-center gap-2">
                <div className={`w-10 h-10 rounded-full flex items-center justify-center ${designMethod === 'design-online' ? 'bg-blue-500' : 'bg-gray-100'}`}>
            ...
            <button onClick={() => setDesignMethod('upload-artwork')} className={`p-4 rounded-xl border-2 transition-all text-center ${designMethod === 'upload-artwork' ? 'border-blue-500 bg-blue-50' : 'border-gray-200 hover:border-gray-300 bg-white'}`}>
              <div className="flex flex-col items-center gap-2">
                <div className={`w-10 h-10 rounded-full flex items-center justify-center ${designMethod === 'upload-artwork' ? 'bg-blue-500' : 'bg-gray-100'}`}>
          ...
          {designMethod === 'upload-artwork' && <div className="mt-6">
              <div className="border-2 border-dashed border-gray-300 rounded-xl p-6 text-center hover:border-blue-500 transition-colors bg-gray-50">
                <input type="file" accept=".pdf,.jpg,.jpeg,.png,.ai,.psd" onChange={handleMultiFileUpload} className="hidden" id="multi-file-upload" multiple />
                <label htmlFor="multi-file-upload" className="cursor-pointer">
                  {isUploadingFiles ? <Loader2 className="w-10 h-10 mx-auto text-blue-500 animate-spin mb-3" /> : <Upload className="w-10 h-10 mx-auto text-gray-400 mb-3" />}
              ...
              {uploadedFiles.length > 0 && <div className="mt-4 space-y-2">
                  ...
                  {uploadedFiles.map((file, index) => <div key={index} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg border border-gray-200">
                      <div className="flex items-center gap-3">
                        <FileText className="w-5 h-5 text-blue-500" />
                        <span className="text-sm text-gray-700 truncate max-w-[200px]">{file.name}</span>
                      </div>
                      <button onClick={() => removeUploadedFile(index)} className="p-1 text-gray-400 hover:text-red-500 transition-colors">
                        <X className="w-4 h-4" />
                      </button>
                    </div>)}
                </div>}
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
        ...
        <div className="flex items-center gap-3">
          <button onClick={() => setQuantity(Math.max(1, quantity - 1))} className="w-12 h-12 rounded-lg border border-gray-300 flex items-center justify-center text-xl font-medium text-gray-600 hover:bg-gray-100 transition-colors">
            −
          </button>
          <Input type="number" value={quantity} onChange={e => setQuantity(Math.max(1, Number(e.target.value)))} min={1} className="w-24 h-12 text-lg text-center" />
          <button onClick={() => setQuantity(quantity + 1)} className="w-12 h-12 rounded-lg border border-gray-300 flex items-center justify-center text-xl font-medium text-gray-600 hover:bg-gray-100 transition-colors">
            +
          </button>
        </div>
      ...
      <div ref={priceCardRef} className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
        ...
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <div className="flex items-baseline gap-3">
              ...
              {calculatedPrice.isOnSale && <div className="text-lg text-gray-400 line-through">
        ...
        <div className="flex flex-col gap-3">
          {product.has_design_tool !== false ? designMethod === 'design-online' ? <Button size="lg" className={`w-full h-14 text-white text-lg font-semibold ${calculatedPrice.isOnSale ? 'bg-red-600 hover:bg-red-700' : 'bg-green-600 hover:bg-green-700'}`} onClick={startDesigning}>
                Design Online
                <Palette className="w-5 h-5 ml-2" />
              </Button> : <Button size="lg" className={`w-full h-14 text-white text-lg font-semibold ${calculatedPrice.isOnSale ? 'bg-red-600 hover:bg-red-700' : 'bg-green-600 hover:bg-green-700'}`} onClick={handleAddToCartWithUploads} disabled={uploadedFiles.length === 0}>
                <ShoppingCart className="w-5 h-5 mr-2" />
                Add to Cart - ${calculatedPrice.total}
              </Button> : <Button size="lg" className={`w-full h-14 text-white text-lg font-semibold ${calculatedPrice.isOnSale ? 'bg-red-600 hover:bg-red-700' : 'bg-green-600 hover:bg-green-700'}`} onClick={handleAddToCart}>
              Add to Cart - ${calculatedPrice.total}
            </Button>}
        </div>
      </div>
        ...
        <DialogContent className="max-w-md">
          ...
          <div className="space-y-4 py-4">
            ...
            <div className="border-2 border-dashed border-gray-300 rounded-xl p-8 text-center hover:border-blue-500 transition-colors">
              <input type="file" accept=".pdf,.jpg,.jpeg,.png,.ai,.psd" onChange={handleFileUpload} className="hidden" id="file-upload" />
              <label htmlFor="file-upload" className="cursor-pointer">
                {isUploading ? <Loader2 className="w-10 h-10 mx-auto text-blue-500 animate-spin mb-3" /> : uploadedFileUrl ? <Check className="w-10 h-10 mx-auto text-green-500 mb-3" /> : <Upload className="w-10 h-10 mx-auto text-gray-400 mb-3" />}
                <div className="font-medium text-gray-700">
            ...
            {uploadedFileUrl && <Button className="w-full bg-green-600 hover:bg-green-700" onClick={handleAddToCart}>
                Add to Cart - ${calculatedPrice.total}
              </Button>}
